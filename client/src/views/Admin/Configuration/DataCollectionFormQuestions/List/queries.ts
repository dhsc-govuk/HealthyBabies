...
export const getModuleByCodeWithFields = async (moduleCode: string): Promise<FormModuleWithFields> => {
  const response = await axios.get<FormModuleWithFields>(`/data-collection-form-questions/modules/code/${moduleCode}`);
  return response.data;
};
